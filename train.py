...
def build_model(vocab_size, padid):
    ...
    #  encoder.cnn = encoder.cnn.to(device)
    decoder = Decoder(
        vocab_size,
        args.embedding_size,
        args.hidden_size,
...
def build_optimizer(encoder, decoder):
    optimizer = optim.Adam(list(encoder.parameters()) + list(decoder.parameters()), args.lr)
    return optimizer


def train_epoch(encoder, decoder, data_loader, criterion, optimizer):
    ...
    for epoch in range(1, 1 + args.epochs):
        for i, (images, captions, lengths) in enumerate(data_loader):
            ...
            images_feature = encoder(images)
            outputs = decoder(captions, lengths, images_feature)
